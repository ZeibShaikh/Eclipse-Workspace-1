...
public class register extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Connection con=DriverManager.getConnection(url+db,username,password1);
			String query="insert into register1(name,email,password,gender,city) values(?,?,?,?,?)";
			...
			e.printStackTrace();
			resp.setContentType("text/html");
			out.print("User not registered!"+e);
			RequestDispatcher rd=req.getRequestDispatcher("/myForm.jsp");
			rd.include(req, resp);
		}
	}
